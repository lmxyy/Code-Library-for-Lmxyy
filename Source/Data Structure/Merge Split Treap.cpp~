...
inline int rnd() { static int x = 1; return x = (x*23333+233); }
inline int rnd(int n) { int x = rnd(); if (x < 0) x = -x; return x%n+1; }
...
struct Node
{
	...
	inline Node() = default;
	inline Node(int v,Node *_mn):size(1),key(rnd()),val(v),mn(_mn) { ch[0] = ch[1] = NULL; }
}pool[maxn*50/4],*root[maxn],*cur;
struct Status
{
	int l,r; ll val;
